...
# Define function to load HDB resale data from ChromaDB based on the user query
def query_chromadb(query):
    # Set up embeddings for ChromaDB vector search
    embeddings = OpenAIEmbeddings(
        model="text-embedding-3-large",
        api_key=os.getenv("OPENAI_API_KEY"),
    )

    # Initialize ChromaDB vector store for similarity search with embeddings
    vector_store = Chroma(
...
# Cache the dataset for improved performance on repeated requests
@st.cache_resource
def load_data():
...
# Define function to query the Language Learning Model (LLM) for advice or insights
def query_llm(prompt):
    llm = ChatOpenAI(
        model="gpt-4o",
        temperature=0,  # Use a deterministic response
        max_tokens=None,
        timeout=None,
        max_retries=2,
        api_key=os.getenv("OPENAI_API_KEY"),
    ...
    # Construct LLM message with system role and user prompt for specialized assistance
    message = [
        (
            "system",
    ...
    # Invoke LLM with constructed prompt and return response
...
# Function to provide personalized advice based on user input
def personalized_advice():
    ...
    if st.button("Get Personalized Advice"):
        ...
        # Call LLM with the constructed prompt and display response
        response = query_llm(prompt)
        st.write(response.content)

# Function to answer user questions about HDB resale market trends
def market_insights():
...
# Function to display the methodology
